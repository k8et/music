...
const Discover: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-start h-screen w-full p-2">
      <div className="flex w-full mb-4 justify-between pl-4 pr-4">
        <label
          htmlFor="genreSelect"
      ...
      <ScrollableContainer>
        <div className="flex flex-wrap gap-8 w-full p-4 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer">
          {tracks.map((track: any, index) => (
            <div
              className={
                "p-4 bg-lightGray3 shadow-lg rounded-lg w-56 track-card"
              }
              key={track.key}
              onClick={() => {
                dispatch(
